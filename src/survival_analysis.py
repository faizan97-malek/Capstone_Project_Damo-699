...
class SurvivalSpec:
    ...
    min_duration: float = 0.1  # We have replaced zero durations with this small value to avoid issues in KM/Cox models.
...
def build_survival_frame(
    ...
    # We avoid zero-duration rows (KM/Cox can behave badly with zeros)
    out.loc[out["duration"] == 0, "duration"] = spec.min_duration

    # We ensure int for event
